...
def main(file, pdf_file):
    ...
    if not cnpj or not valor_total or not volume_total or not data_emissao or not data_inicio or not data_fim or not numero_fatura or not valor_icms or not correcao_pcs:
        ...
        if verificar:
            data_frame = dados_excel(cnpj, valor_total, volume_total, data_emissao, data_inicio, data_fim, numero_fatura, valor_icms, correcao_pcs, dist, file)
            sucesso = adicionar_dados_excel(caminho_excel, data_frame)
            if sucesso:
                mover_faturas_lidas(pdf_file, diretorio_destino)
            else:
                print(f"Falha ao adicionar dados na planilha para o arquivo {pdf_file}")
        else:
